...
| signature       | String | 子设备绑定签名串。 签名算法：<br/>1. 签名原串，将产品 ID 设备名称，随机数，时间戳拼接：`text=${productID}${deviceName};${random};${expiration_time}`<br/>2. 使用设备 Psk 密钥，或者证书的 Sha1 摘要，进行签名：`base64_encode(hmac_sha1(device_secret, text))` |
| random          |  Int  | 随机数                                                                                                                                                                                                                                                         |
| timestamp       |  Int  | 时间戳，单位：毫秒                                                                                                                                                                                                                                             |
